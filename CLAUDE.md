...
- **Epic 0 scope (approved, not yet built):** the foundation layer — `backend/`/`frontend/`/`deployment/` monorepo, CI/CD, containers, execution tracing, PHI redaction, and the settled third-party stack. Designed *after* Epic 1 but must be **built before Epic 1 M4**, or M4's files get moved twice.
...
- [docs/epic0_foundation/](docs/epic0_foundation/README.md) — the foundation layer, approved 2026-08-14. `M1_repo_restructure.md`, `M2_observability.md`, `M3_epic1_handoff.md`. **M1 and M2 are full task plans with TDD steps — they are executable as written.** `M3` is a planning boundary: it records the deltas Epic 0's stack decisions impose on Epic 1 M3–M8, and defers each of those modules to its own plan cycle.
