...
- **Epic 0 scope (M1 built 2026-08-15; M2–M3 planned):** the foundation layer — `backend/`/`frontend/`/`deployment/` monorepo, CI/CD, containers, execution tracing, PHI redaction, and the settled third-party stack. Designed *after* Epic 1 but must be **built before Epic 1 M4**, or M4's files get moved twice.
...
- The roadmap is 5 **Epics** (top-level units, former "Phases"), each decomposed into numbered **Modules** (M1, M2, … — implementation-sized units, former "Slices").

Every epic folder follows one shape: an overview `README.md` (goals, decisions, architecture, build order, cross-cutting concerns, repository layout) plus one doc per module; when implementing or reviewing a module, load its module doc.

- [docs/epic0_foundation/](docs/epic0_foundation/README.md) — the foundation layer, approved 2026-08-14. `M1_repo_restructure.md`, `M2_phi_redaction.md`, `M3_execution_tracing.md`, `M4_epic1_handoff.md`. **M1 is implemented; M2 and M3 are full task plans with TDD steps, executable as written.** `M4` is a planning boundary: it records the deltas Epic 0's stack decisions impose on Epic 1 M3–M8, and defers each of those modules to its own plan cycle.
- [docs/epic1_grounded_qa/](docs/epic1_grounded_qa/README.md) — the authoritative spec for the current code: `M1_ingestion_pipeline.md` … `M8_eval_harness.md`. These are *contracts*, not plans; a module's plan lands beside it as `M<N>_<name>_plan.md` when that module is picked up.

The Epic 2+ vision is designed (not yet built) in an epic-doc series: [Epic 2](docs/Epic2_first_actions.md) (calendar + reminders — also defines the shared agent/tool/confirmation-gate foundations), [Epic 3](docs/Epic3_external_actions.md) (email/delivery/pharmacy via assisted handoff), and [Epic 4](docs/Epic4_portal_and_proactive.md) (Epic on FHIR + proactive). Each epic doc is self-contained for that epic.
...
- **Tracing is opt-in and redacted at set time.** [backend/src/tara/execution_tracing/](backend/src/tara/execution_tracing/) wraps OpenTelemetry; `traced_span()` in [span_emitter.py](backend/src/tara/execution_tracing/span_emitter.py) is the only span-creation path, and `span_redaction.py` runs every string attribute through [phi_redaction.py](backend/src/tara/phi_redaction.py) before the value reaches the span. Instrumentation is vendor-neutral OTLP, so the backend (Phoenix by default, self-hosted as a Compose service on 6006; Langfuse a documented swap) is an endpoint change, never a code change.
