...
- **Storage is local SQLite + sqlite-vec.** [src/tara/local_data_stores/metadata_db.py](src/tara/local_data_stores/metadata_db.py) holds the relational schema (documents, chunks, queries); [src/tara/local_data_stores/vector_index.py](src/tara/local_data_stores/vector_index.py) holds the `vec0` virtual table keyed by `chunk_id`. The vector table is created separately from the main schema because it needs the sqlite-vec extension loaded. Embedding dimension comes from config (`embed_dim`, must match `embed_model`).
