...
3. Start a fresh brainstorm → plan cycle for **M4 only**, using `docs/epic1_grounded_qa/M4_grounded_answering.md` as the contract and §3.1 and §3.2 above as the deltas.
   - **Done 2026-08-26**, with two rulings that supersede the deltas above — see the module doc's "As built" section:
     - **§3.2's reranker was deferred out of M4 into its own plan cycle.** `chunk_reranker.py`, the four `TARA_RERANK_*` settings and the second abstention route are untouched. Reason: the reranker is a retrieval-quality change with a threshold M8 has not calibrated yet (§3.2 row 5 says so itself), and bundling it into M4 would have made the grounding, citation and abstention work depend on an uncalibrated score.
     - **§3.1 row 1's Instructor prescription was replaced by native schema-constrained generation.** `generate_structured_object()` exists as specified and still returns a validated Pydantic model; what changed is what constrains the model — each backend's own JSON-schema facility (the OpenAI-compatible `strict` response format, Ollama's `format`, the Agent Platform client's structured output) behind one new `LLMClient.generate_structured_json()` method, with validation and retry in `structured_completion.py`. Reason: no new dependency, one validation point rather than a library's, and nothing to remove when LangGraph lands (§4.3 row 3).
