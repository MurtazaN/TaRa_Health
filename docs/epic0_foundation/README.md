...
- Two containers — `tara-backend` and `phoenix` (the trace viewer, added by M3).
- **LM Studio is not one of them.** It stays on the host because it needs direct graphics-hardware access; the backend container reaches it via `host.docker.internal`.
- Phoenix must be given `PHOENIX_WORKING_DIR=/mnt/data`. Unset, it writes its store to `$HOME/.phoenix` inside the container, the `phoenix-data` volume receives nothing, and PHI-derived span data lands in an unmanaged writable layer.
...
- Spec [§10](M5_model_backends.md#10-acceptance) asks for "all 12 assertions in §9.3 present and passing". **All twelve pass as of Epic 1 M4 (2026-08-26)**; eleven passed at M5, with assertion 9 held as a strict `xfail` pending M4. The container check has been run and passes — measurements below. Recorded here rather than implied by a green suite.
...
| 9 | Emergency question returns `safety_flag: "emergency"` and HTTP 200 while every Agent Platform call raises | **Passing as of Epic 1 M4 (2026-08-26).** It was recorded here as `xfail(strict=True)` because `screen_for_emergency()` was still an Epic 1 stub raising `NotImplementedError`. Epic 1 M4 filled that stub with the keyword pass, the strict xfail flipped to a genuine pass, and the marker is gone: `test_real_answer_path_triages_emergency_while_agent_platform_is_down` now runs the real path green. **All twelve §9.3 assertions pass.** The sibling test covers response shaping only and no longer claims §4.3 |
