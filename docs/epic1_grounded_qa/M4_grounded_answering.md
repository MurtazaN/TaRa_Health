...
| 4 | No audit row is written when generation raises — retry budget exhausted, credentials missing, or the hosted service unavailable — although the module docstring and the plan both state every terminal outcome is recorded | **OPEN, awaiting a decision.** A request that attempted egress and then failed leaves no local record that the attempt happened. `test_every_outcome_writes_one_audit_row` is named as though it covers this and does not: it parametrizes answered, declined and emergency only |
